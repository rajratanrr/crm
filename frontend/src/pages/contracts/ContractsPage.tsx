    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200">
        ...
        <div className="overflow-x-auto"><table className="w-full"><thead><tr className="border-b border-gray-100">
          {['Contract', 'Customer', 'Event', 'Package', 'Amount', 'Paid', 'Remaining', 'Status'].map(h => <th key={h} className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase">{h}</th>)}
        </tr></thead><tbody>
          {contracts.map((c: any) => (
            <tr key={c.id} onClick={() => navigate(`/contracts/${c.id}`)} className="border-b border-gray-50 hover:bg-gray-50 cursor-pointer">
              ...
              <td className="px-4 py-3 text-sm text-gray-600">{c.event?.eventName || '-'}</td>
